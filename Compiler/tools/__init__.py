...
class SyntacticAnalyzer():
	def __init__(self, loading, family):
		...
		self.GOTO = {y.name: {x: ' ' for x in self.Nonterminal}
					for y in self.itemSets}
	...
	def isRecognizable(self, origin):
		...
		while True:
			...
			self.parseRst.append({'stateStack': copy.deepcopy(
				stateStack), 'shiftStr': copy.deepcopy(shiftStr), 'inputStr': copy.deepcopy(inputStr)})
		...
		if nt == 'program':
			...
			if len(r) == 3:
				...
				if s != None:
					...
					self.grammarErrMsg = "变量重定义：" + \
						str(shiftStr[-2]['row']) + "行" + str(shiftStr[-2]['colum']) + "列"
					return
				else:
					s = Symbol()
				if n.place == None:
			...
			if len(r) == 1 and r[0]['type'] == 'INT':
				...
				if not function:
					...
					self.grammarErrMsg = "未定义的函数：" + \
											shiftStr[-4]['data'] + "，在" + \
											str(shiftStr[-4]['row']) + "行" + \
											str(shiftStr[-4]['colum']) + "列"
					return
				...
				if not n:
					...
					self.grammarErrMsg = "未定义的变量：" + \
											shiftStr[-1]['data'] + "，在" + \
											str(shiftStr[-1]['row']) + "行" + \
											str(shiftStr[-1]['colum']) + "列"
					return
...
class ObjectCodeGenerator():
	...
	def genMips(self):
		...
		mc.append('addiu $sp, $zero, 0x{}'.format(
			self.DATA_SEGMENT + self.STACK_OFFSET))
